...
| 场景 | 恢复方式 |
|------|---------|
| 检查点失败 | 修复该步骤的问题，只重新运行该步骤及其后续步骤 |
| Checker 遗漏 | 读取 Checker 的具体错误报告，只修改出错的步骤 |
| 逻辑错误 | 回溯到引入错误的最早步骤，从那一步开始重做 |

> 📌 **详细说明**：完整的 Worker 执行策略详见 `ARCHITECTURE.md` 的 "Worker 执行策略" 章节。

...
```
❌ 大步骤：修复认证 bug（30 行代码，容易遗漏）
✅ 细粒度：
   1. 读取 auth.py，理解当前逻辑（0.5 行）
   2. 定位 token 过期检查的位置（1 行）
   3. 添加刷新逻辑（2 行）
   4. 添加日志输出（0.5 行）
   5. 运行测试验证（1 行）
```
...
**拆分原则：**
- 每个步骤只做**一件事**（如果能用"和"连接两个动作，说明需要拆）
- 每个步骤的完成状态是**可验证的**（运行测试/读取输出/对比预期）
- 每个步骤应该**不需要记住上一步的细节**（文件组织好、中间产物清晰）

#### 2. 进度检查点（Checkpoints）

在关键步骤后立即验证，不要等到最后才发现中间出了错：

```
步骤 1 → 完成 → 检查点 1（运行语法检查？读取输出？）
步骤 2 → 完成 → 检查点 2（单元测试通过？）
步骤 3 → 完成 → 检查点 3（集成测试通过？）
...
最后 → 自检清单
```
...
| 步骤类型 | 检查方式 |
|---------|---------|
| 写代码 | `python -m py_compile` 语法检查 |
| 生成文件 | 读取文件验证格式、大小、关键内容 |
| 修改配置 | 重新加载验证配置生效 |
| 执行命令 | 检查返回码 + 读取 stderr |
| API 调用 | 验证返回状态码 + 数据结构 |
...
```
run_output/
├── original/        ← 原始文件（绝不动，用于重试对比）
├── steps/           ← 中间步骤产物（带序号）
│   ├── step1_xxx.json
│   ├── step2_xxx.json
│   └── step3_xxx.json
├── checkpoints/     ← 检查点快照（用于快速回滚）
└── final_output     ← 最终提交给 Checker 的文件
```

**为什么这样做：**
- 重试时能快速定位哪步出错（比全量重做快 10 倍）
- Checker 验证时只需要看 `final_output`
- 原始文件不变，保证重试的基线一致

#### 4. 错误预检（Pre-submit Checks）

在写入 `final_output` 之前，必须过一遍：

```
□ 读取 final_output，确认是预期格式
□ 检查必填字段是否都有值
□ 检查文件大小是否合理（不能是 0 字节或异常大）
□ 检查关键内容是否存在（比如：要求的 JSON key 是否存在）
□ 如果是代码文件：语法检查
□ 如果是文档文件：关键段落是否完整
□ 如果是数据文件：数据行数是否符合预期
```
...
**失败后怎么办：**
- 不提交有问题的文件，而是在 `steps/` 里修复中间步骤
- 只有全部预检通过才写入 `final_output`

#### 5. 上下文管理

Worker 在执行过程中需要保持清晰的状态：
...
进行中：
  → 步骤 4：执行遮盖（正在处理第 3 列）
待做：
  - 步骤 5：保存到 final_output
  - 步骤 6：运行预检
```

**好处：**
- 被中断或重试时能快速恢复
- 可以清晰地告诉 Checker "我完成了哪些"
- 便于自己追踪遗漏

#### 6. 失败恢复模式

当检查点失败时，不是从头重做，而是**定点修复**：
